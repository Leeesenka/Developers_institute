...
const _getAllLogin = (req, res) => {
    ...
    const _insertRegister = async (req, res) => {
        ...
        if (!req.body.data) {
            return res.status(400).json({msg: 'The request body must contain a data object.'});
        }
    
        if (!data.password) {
            return res.status(400).json({msg: 'The data object must contain a password field.'});
        }
        ...
        try {
            ...
            .catch(err => {
              ...
              if (err.message.includes("unique constraint") && err.message.includes("email")) {
                return res.status(400).json({msg: `Email ${data.email} already exists.`});
              }
...
const _getUsernameLogin = async (req, res) => {
  const { data } = req.body;
    ...
    if (!data.username || !data.password) {
        return res.status(400).json({ msg: 'Username and password fields are required.' });
      }
  ...
  try {
    ...
    // сравниваем хэшированный пароль в базе данных с переданным паролем
    const isMatch = await bcrypt.compare(data.password, user.password);

    // если пароли совпадают, возвращаем успешный ответ
    if (isMatch) {
